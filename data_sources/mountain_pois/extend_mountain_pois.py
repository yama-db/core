...
def main():
    ...
    for row in cursor.fetchall():
        ...
        if not (result := cursor.fetchone()):
            print(
                f"No nearby Yamareco POI found for Yamap {raw_remote_id} '{name}', skipping."
            )
            continue
        # 名前の類似度を計算
        ...
        if similarity < 0.8:
            print(
                f"Yamap '{name}' and Yamareco '{result['name']}' similarity {similarity:.2f} is too low, skipping."
            )
            continue
        # unified_poisに登録
        count += 1
