...
CREATE TABLE book_details (
    ...
    jp_number VARCHAR(12) COLLATE ascii_bin COMMENT '全国書誌番号(JPNO)',
    ndl_search_url VARCHAR(255) COLLATE ascii_bin COMMENT 'NDL書誌情報URL',
...
CREATE TABLE administrative_regions (
    ...
    representative_point POINT NOT NULL SRID 4326 COMMENT '代表点',
    INDEX idx_pref_code (pref_code)
) COMMENT '行政区画マスタ(属性および代表点)';
...
CREATE TABLE administrative_boundaries (
    ...
    geom POLYGON NOT NULL SRID 4326 COMMENT '境界ジオメトリ',
...
CREATE TABLE stg_gsi_gazetteer_pois (
    ...
    geom POINT NOT NULL SRID 4326 COMMENT '地理位置',
...
CREATE TABLE stg_gsi_natural_pois (
    source_uuid BINARY(16) PRIMARY KEY COMMENT 'UUID v5',
    raw_remote_id VARCHAR(100) COLLATE ascii_bin COMMENT '元データのID',
    names_json JSON COMMENT '名称配列(別名含む)',
    geom POINT NOT NULL SRID 4326 COMMENT '地理位置',
...
CREATE TABLE stg_book_pois (
    ...
    names_json JSON COMMENT '名称配列(別名含む)',
    geom POINT SRID 4326 COMMENT '地理位置（推定）',
    elevation_m DECIMAL(6, 1) COMMENT '標高',
    -- 個別の属性
...
CREATE TABLE unified_pois (
    ...
    representative_geom POINT NOT NULL SRID 4326 COMMENT '代表座標',
    ...
    min_zoom_level TINYINT NOT NULL DEFAULT 15 COMMENT '表示開始ズームレベル',
    priority_score FLOAT NOT NULL DEFAULT 0 COMMENT '表示優先度スコア',
...
CREATE TABLE poi_links (
    ...
    source_type ENUM('GSI_GAZETTEER', 'GSI_NATURAL', 'GSI_GCP', 'YAMAP', 'YAMARECO', 'BOOK') NOT NULL COMMENT '参照先テーブル種別',
